...
from utils.classifier_metric.evaluator import cls_metric, auc_metric, f1_metric, precision_metric, recall_metric, acc_metric
...
def prepare_batch_inputs_and_labels(batch, tokenizer):
    ...
    # 注意：截断操作将丢弃一些超过最大长度的表和列，截断512以上的表和列
    ...
    # tokenizing后对齐batch_question_ids, batch_column_info_ids, and batch_table_name_ids
    for batch_id in range(batch_size):
    ...
    for epoch in range(opt.epochs):
        ...
        for batch in train_dataloder:
            ...
            if train_step % num_checkpoint_steps == 0:
                ...
                table_pred_probs_for_f1, column_pred_probs_for_f1 = [], []
                ...
                for batch in dev_dataloder:
                    ...
                    for batch_id, table_logits in enumerate(model_outputs["batch_table_name_cls_logits"]):
                        ...
                        table_pred_probs_f1 = torch.argmax(table_logits, dim=1)
                        table_pred_probs_for_f1.extend(table_pred_probs_f1.cpu().tolist())
                    ...
                    for batch_id, column_logits in enumerate(model_outputs["batch_column_info_cls_logits"]):
                        ...
                        column_pred_probs_f1 = torch.argmax(column_logits, dim=1)
                        column_pred_probs_for_f1.extend(column_pred_probs_f1.cpu().tolist())

                # calculate AUC score for table and column classification
                ...
                # calculate f1 score for table and column classification
                table_f1 = f1_metric(table_labels_for_auc, table_pred_probs_for_f1)
                column_f1 = f1_metric(column_labels_for_auc, column_pred_probs_for_f1)

                # calculate acc score for table and column classification
                table_acc = acc_metric(table_labels_for_auc, table_pred_probs_for_f1)
                column_acc = acc_metric(column_labels_for_auc, column_pred_probs_for_f1)

                # calculate precision score for table and column classification
                table_precision = precision_metric(table_labels_for_auc, table_pred_probs_for_f1)
                column_precision = precision_metric(column_labels_for_auc, column_pred_probs_for_f1)

                # calculate recall score for table and column classification
                table_recall = recall_metric(table_labels_for_auc, table_pred_probs_for_f1)
                column_recall = recall_metric(column_labels_for_auc, column_pred_probs_for_f1)

                print("table auc:", table_auc)
                print("column auc:", column_auc)
                print("total auc:", table_auc + column_auc)

                print("table f1:", table_f1)
                print("column f1:", column_f1)
                print("total f1:", table_f1 + column_f1)

                print("table acc:", table_acc)
                print("column acc:", column_acc)
                print("total acc:", table_acc + column_acc)

                print("table precision:", table_precision)
                print("column precision:", column_precision)
                print("total precision:", table_precision + column_precision)

                print("table recall:", table_recall)
                print("column recall:", column_recall)
                print("total recall:", table_recall + column_recall)
                ...
                toral_f1_score = table_f1 + column_f1
