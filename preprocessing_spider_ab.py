...
def main(opt):
    ...
    for natsql_data, data in tqdm(zip(natsql_dataset, dataset)):
        ...
        preprocessed_data["ant"] = data["ant"]  # slsql
        preprocessed_data["ant_tbl_names"] = []  # slsql
        preprocessed_data["ant_col_val_names"] = []  # slsql
        ...
        '''
            此处添加模式链接的id对应关系
        '''
        preprocessed_data["ant_col_val_names"].append({"id": 0, "name": '[*]'})
        for ti, tables in enumerate(db_schemas[db_id]["schema_items"]):
            cols = tables["column_names_original"]
            for ci, column in enumerate(cols):
                column = tables["table_name_original"] + "." + column
                preprocessed_data["ant_col_val_names"].append({"id": len(preprocessed_data["ant_col_val_names"]), "name": column})
        preprocessed_data["ant_col_val_names"].append({"id": len(preprocessed_data["ant_col_val_names"]), "name": "[none]"})

        for tables_id in db_schemas[db_id]["schema_items"]:
            preprocessed_data["ant_tbl_names"].append({"id": len(preprocessed_data["ant_tbl_names"]), "name": tables_id["table_name_original"]})
        preprocessed_data["ant_tbl_names"].append({"id": len(preprocessed_data["ant_tbl_names"]), "name": "[none]"})

        # add database information (including table name, column name, ..., table_labels, and column labels)
        # 添加数据库信息(包括表名称，字段名称，表标签，字段标签)
        for tid, table in enumerate(db_schemas[db_id]["schema_items"]):
            ...
            '''
                在此处实现，Question和表、字段之间的匹配关系，将所有的模式列表与模式链接列表匹配，匹配为1，不匹配则为0
            '''
            tab_link_labels = []
            col_val_link_labels = []
            for ant in preprocessed_data["ant"]:
                if ant is None:
                    continue
                if ant["type"] == "tbl":
                    ant_tbl_name = preprocessed_data["ant_tbl_names"][ant["id"]]  # 根据id查找name
                    tab_link_labels.append(ant_tbl_name["name"])
                else:
                    ant_col_val_name = preprocessed_data["ant_col_val_names"][ant["id"]]  # 根据id查找name
                    col_val_link_labels.append(ant_col_val_name["name"])

            if opt.target_type == "sql":
                if table["table_name_original"] in tab_link_labels:  # for used tables
                    preprocessed_data["table_labels"].append(0)
                else:
                    preprocessed_data["table_labels"].append(0)

                # for unused tables and their columns
                column_labels = []
                for column_name_original in table["column_names_original"]:
                    if column_name_original in col_val_link_labels or \
                        table["table_name_original"] + "." + column_name_original in col_val_link_labels: # for used columns
                        column_labels.append(1)
                        # 如果字段被链接，则字段所在的表也应该被链接
                        preprocessed_data["table_labels"][tid] = 0   # 根据表名获取索引
                    else:
                ...
                for column_name_original in table["column_names_original"]:
                    if column_name_original in col_val_link_labels or \
                        ...
                        column_labels.append(1)
                        # 如果字段被链接，则字段所在的表也应该被链接
                        preprocessed_data["table_labels"][tid] = 1   # 根据表名获取索引
                    else:
                        column_labels.append(0)
                preprocessed_data["column_labels"].append(column_labels)
            else:
