...
with open('./data/spider/train_spider.json') as spider, open('./data/slsql/spider_ant_train.json') as spider_ant:
    spider = list(json.load(spider))
    spider_ant = list(json.load(spider_ant))
    ret = []
    for i, (ex, a) in enumerate(zip(spider, spider_ant)):
        cur = {'id': i, 'db_id': ex['db_id'], 'query': ex['query'], 'query_toks': ex['query_toks'], 'query_toks_no_value': ex['query_toks_no_value'], 'question': ex['question'], 'ant': a}
        ret.append(cur)
    fix_issues(ret)
    with open('./data/preprocessed_data/slsql_train.json', 'wt') as output:
        json.dump(ret, output, sort_keys=True, indent=4, separators=(',', ': '))

with open('./data/spider/dev.json') as spider, open('./data/slsql/spider_ant_dev.json') as spider_ant:
    spider = list(json.load(spider))
    spider_ant = list(json.load(spider_ant))
    ret = []
    for i, (ex, a) in enumerate(zip(spider, spider_ant)):
        cur = {'id': i, 'db_id': ex['db_id'], 'query': ex['query'], 'query_toks': ex['query_toks'], 'query_toks_no_value': ex['query_toks_no_value'], 'question': ex['question'], 'ant': a}
        ret.append(cur)
    with open('./data/preprocessed_data/slsql_dev.json', 'wt') as output:
        json.dump(ret, output, sort_keys=True, indent=4, separators=(',', ': '))
